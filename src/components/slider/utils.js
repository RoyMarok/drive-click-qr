...
export const getValuePrefixSuffix = (value, prefix = '', suffix = '') => `${prefix}${value}${suffix}`

export const getBoundaries = ({ options, grid, min, max }) => {
    if (options) {
        return {
            min: _.first(options).title,
            max: _.last(options).title
        }
    } else if (grid) {
        return {
            min: _.first(grid),
